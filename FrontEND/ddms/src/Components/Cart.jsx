        ...
        <>
            <div className={`fixed right-0 top-0 w-full lg:w-[20vw] h-full p-5 bg-white mb-3 ${activeCart?"translate-x-0":"translate-x-full"} transition-all duration-500 z-50`}>
                ...
                cartItems.length>0 ? cartItems.map((medicine)=>{
                    return (<CartMedicine key={medicine.id} 
                    id={medicine.id}
                    name={medicine.name}
            ...
            <HiOutlineShoppingCart onClick={()=>setActiveCart(!activeCart)} className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-4 right-4 ${totalQty>0 && "animate-bounce delay-500 transition-all"}`}/>
        </>
    )
}
